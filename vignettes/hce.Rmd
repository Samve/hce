...
date: "`r format(as.Date('2025-12-01'), '%d %B, %Y')`"
author:
...
1. The most prioritized outcome
...
2. Sequential comparison (see @poc2012)
...
Here we describe the function `simKHCE()` which is used to simulate a kidney HCE with 6 components, defined in the following order: 

1. Kidney Failure Replacement Therapy (KFRT); 

1. Sustained eGFR < 15; 

1. Sustained 57 percent or more decline in eGFR; 

1. Sustained 50 percent or more decline in eGFR; 

1. Sustained 40 percent or more decline in eGFR; 
...
The simulation is conducted over a fixed follow-up duration $[0, T]$, where the maximum duration $T$ (in years) is specified in the variable `fixedfy` (fixed follow-up years). The duration is divided into $m$ equidistant visits for each patient. The model is a mixed effects model with a random slope and a random intercept which are independent from each other
$$y_{ij}=\alpha_i+\{\beta_i + [b^1 - (\beta_i)^-\varphi]x_i\}t_j+\varepsilon_{ij}, \ \ t_j=\frac{T}{m}j,\,j=0,1,\cdots,m,\,i=1,\cdots,N=n_0+n.$$

Here $n$ is the number of patients in the active group, $n_0$ the number of patients in the placebo group, $x_i$ is the indicator for the active group (1 if the patient is in the active group, 0 otherwise). 

- The baseline eGFR values are randomly sampled from the uniform distribution $\alpha_i\sim {\mathcal U}[E_{\min}, E_{\max}].$ 

- The random slope in the control (placebo) group is simulated from the normal distribution $\beta_i\sim {\mathcal N}(b^0, \Sigma^2).$ Here $b^0$ is the mean placebo progression rate specified in the argument `CM_P.` The positive and negative parts $(\beta_i)^+=\max(\beta_i, 0)$ and $(\beta_i)^-=\max(-\beta_i, 0)$ are used to denote the fact that when the slope is negative for the patients in the active groups, then the treatment effect acts both through multiplicative and additive components
$$\beta_i + [b^1 - (\beta_i)^-\varphi]x_i=\beta_i (1-\varphi)+b^1.$$
While for patients with a positive slope, the treatment effect is only additive:

$$\beta_i + [b^1 - (\beta_i)^-\varphi]x_i=\beta_i +b^1.$$
The additive effect of the active groups is $b^1$ and is specified in `CM_A`.

- $\Sigma$ is the between-patient variability of progression slopes over time, specified in the variable `Sigma.` 

- The random noise $\varepsilon_{ij}\sim {\mathcal N}(0, \sigma^2),$ where $\sigma$ specifies the within-patient variability provided in `sigma.`
