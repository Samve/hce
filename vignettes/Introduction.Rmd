---
title: "Introduction"
date: "`r format(as.Date('2026-01-15'), '%d %B, %Y')`"
author:
...
The package simulates and analyzes hierarchical composite endpoints (HCEs). The primary analysis method is win odds, and other win statistics—win ratio and net benefit [@dong2023win]—are implemented when there is no censoring. Win odds relies on the DeLong–DeLong–Clarke-Pearson formula [@delong1988comparing] for the variance of the win proportion and is based on the Brunner–Munzel test [@brunner2000nonparametric]. We also include implementations of the Brunner–Konietschke version [@brunner2025unbiased] of the Bamber estimator [@bamber1975area] for the variance of the win proportion. In addition, we provide Wilson-type, range-preserving confidence intervals for the win proportion and win odds, following Schüürhuis, Konietschke, and Brunner [@schuurhuis2025new].
...
7. Threshold-adjusted win odds calculation:

- `deltaWO()`.

8. Power, sample size, and minimum detectable win odds calculation functions:
...
9. Print and plot methods for `hce_results` objects, generated by the functions `powerWO(), sizeWO(), minWO()`. 

10. A plot method for `hce` objects (created by `as_hce()`) to provide the ordinal dominance graph [@bamber1975area].

11. The function `IWP()` to calculate patient-level individual win proportions [@gasparyan2021adjusted;@gasparyan2021power].
